...
export default observer(function PriceCalculator() {
    ...
    return (
        <FormBox title="Today's Fuel Price" titleStyle={{ fontSize: 18 }} titleAction={<ImperialSwitch />}>
            <Grid container spacing={4}>
                <Grid item xs={12} display="flex" alignItems="center">
                        <Typography sx={{fontSize: 17}}>Current Price: </Typography>
                        <Typography sx={{pl: 1, fontSize: 18, fontWeight: 600}}>Rs. {imperial ? getImperialGallonPrice(getFuelPrice()) : getFuelPrice()}</Typography>
                    {/* <Box paddingLeft={1}>
                        <Typography>
                            {'Rs. ' + (imperial
                ...
                <Grid item xs={10}>
                    <TextField
                        ...
                        type="number"
                        inputProps={{ min: 0 }}
                        placeholder={`Enter Fuel Quantity in ${imperial ? "Gallon" : "Litre"} `}
                        variant="standard"
                        fullWidth
                    />
                </Grid>
