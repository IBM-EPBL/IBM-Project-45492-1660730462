...
from utils.helper import token_required
...
def get_profile_details():
    ...
    return jsonify({'name': user.name, 'email': user.email, 'mobile': user.mobile}), 200
...
def update_profile():
    ...
    try:
        db.session.add(user)
        db.session.commit()
        return msgResponse("Successfully updated profile"), 200
    except:
        db.session.rollback()
